...
2. 開発中バージョンの開発計画（現在: `docs/v0.2/01_development-plan.md`。過去バージョンの計画・仕様は `docs/v0.1/` 等に置かれている）
...
2. 設計判断（クレート追加、仕様変更、アルゴリズム選択）を行ったら開発中バージョンの `docs/<version>/decisions/`（現在: `docs/v0.2/decisions/`）に ADR を 1 件追加する。「なぜそうしたか」「却下した代替案」を必ず書く
