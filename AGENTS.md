...
2. 開発中バージョンの開発計画（現在: `docs/v0.2/10_v0.2-development-plan.md`。過去バージョンの計画・仕様は `docs/v0.1/` 等に置かれている）
...
3. **unsafe の隔離**: `unsafe` は `hook.rs` / `sender.rs` / `window.rs`、および IME インジケーター関連の `src/ime_indicator/`（detect.rs / overlay.rs）と検証用 `examples/ime_probe.rs` のみ（ADR 0020）。各 unsafe に `// SAFETY:` コメント必須
