...
3. **unsafe の隔離**: `unsafe` は `hook.rs` / `sender.rs` / `window.rs`、IME インジケーター関連の `src/ime_indicator/`（detect.rs / overlay.rs）と検証用 `examples/ime_probe.rs`（ADR 0020）、およびコンソール接続・通知ダイアログの `notify.rs`（ADR 0031）のみ。各 unsafe に `// SAFETY:` コメント必須
