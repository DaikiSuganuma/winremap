...
2. `docs/02_development-plan.md` — フェーズ別開発計画
3. `docs/03_rust-guidelines.md` — Rust 開発の作法
4. `docs/decisions/` — 全 ADR（Architecture Decision Record: 設計判断とその理由の記録。1 判断 1 ファイル）
...
2. **フックコールバック内の処理制限**: コールバック内でのヒープ確保、ロック待ち、ファイル I/O、ログ出力、重い Win32 API 呼び出しを禁止。設定は事前構築の読み取り専用構造を参照するのみとし、リロードは atomic swap（`arc-swap` 等）で行う。明示的な例外は 2 つのみ: `--debug` 時の自スレッドへの `PostThreadMessageW`（ADR 0016）と、利用者が `--macro-delay` を指定した場合の有界な sleep（上限 15ms × 8、ADR 0018）
3. **unsafe の隔離**: `unsafe` は `hook.rs` / `sender.rs` / `window.rs` のみ。各 unsafe に `// SAFETY:` コメント必須
...
2. 設計判断（クレート追加、仕様変更、アルゴリズム選択）を行ったら `docs/decisions/` に ADR を 1 件追加する。「なぜそうしたか」「却下した代替案」を必ず書く
...
- ブリーフに無い大機能の先行実装（IME 対応等の Non-goals は提案のみ可、実装は不可）
