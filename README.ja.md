...
- **カーソルの色が、まれに片方だけ、あるいはまったく着かないことがあります。** IME がオンのあいだ、色付きの I ビームが出ないことがあります。矢印のほうは正常で、IME のパネルも出ます。**WinRemap を起動し直すと直ります。** v0.8 の受け入れでは「何も描かれない」形で 1 台の機械に 3 回、v0.9 の受け入れでは「着色そのものが入らない」形で 1 度起きました。狙って再現できたことは一度もなく、原因は未特定です。0.9.0 で**中身が空のカーソルを組み込まないようにした**ため、以前あった「IME を切り替えても戻らない」状態にはなりません。また、**着けられなかったカーソルがあるときは、その数と理由が `--debug` のログに残り、IME をオンにするたびに言い直されます**。遭遇された場合はそのログを添えて報告いただけると助かります。なおこの設定は既定で無効です
