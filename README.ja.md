...
キー名は、あなたのキーボードに合わせて出ます（`Num1 (0x61)`、記号キーはその配列が刻んでいる文字）。ASCII 制御コードを持つキー・コマンドは、それも表示します（`C-h (BS 0x08)`、`Enter (CR 0x0D)`）。**英数字にはコードを付けません** — WinRemap が記録するのはキーであって、打った内容ではないからです。ディスクにも書きません。
...
- **記号キー（OEM キー。OEM = Original Equipment Manufacturer）**（`;` `,` 等）はまだ規則に書けません（VK（Virtual-Key）コードがキーボードレイアウト依存のため）。ログには名前が出るので（`/ (0xBF)`）、どのキーを押したかは確認できます
...
- ターミナルから `--debug` を付けて起動した場合、出力はそのターミナルに表示されますが、プロンプトはすぐ戻るため出力がプロンプトと混ざって見えます。**このターミナルを閉じると WinRemap も終了します** — `--debug` はログをそこへ流し続けるためコンソールに接続したままであり、Windows はコンソールを閉じるときに接続中のプロセスをすべて終了させるためです。`--debug` を付けない場合は何も出力せず、起動完了時にコンソールを切り離すため、ターミナルを閉じても動き続けます
