...
トレイアイコンを右クリックして「設定」を選ぶと、WinRemap が現在参照している設定が表示されます。キーマップごとの対象アプリ・除外アプリ・リマップ規則が、設定ファイルの同じ行に書いたコメント付きで並びます。同じ入力を複数のキーマップが束縛している場合は、相手のキーマップ名を示す列が出ます（実際に効くのは 1 つだけのためです）。キー記法の凡例も横のペインに表示されます。
