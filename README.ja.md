...
- **カーソルの色が、まれにまったく描かれなくなることがあります。** IME がオンのあいだ、色付きの I ビームが出なくなることがあります（色が消えるのではなく、何も描かれません）。矢印のほうは正常で、IME のパネルも出ますが、IME を切り替えても戻りません — **WinRemap を起動し直すと戻ります**。v0.8 の受け入れで 1 台の機械に 3 回起きましたが、狙って再現できたことは一度もなく、原因は未特定です。`--debug` はカーソルの差し替えを毎回記録するので、遭遇された場合はそのログを添えて報告いただけると助かります。なおこの設定は既定で無効です
