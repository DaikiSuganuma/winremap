...
- **ログウィンドウを開いている間、切り替えた先のアプリが前面アプリ行に出ない**（2026-07-29、`00-log-view` に前面アプリ行の検査を足したときに 2 回の実行で観測）。報告されたのは `winremap.exe` と `explorer.exe` だけで、**Notepad は一度も出なかった** — `GetForegroundWindow()` で前面であることを確認し、そのキーがフックに届いている（判定行が出ている）にもかかわらず、である
  ...
  - **2026-08-01、`06-foreground-line` で決着した。観測は再現し、しかも表示だけの問題ではなかった。**

    | 状態 | 切り替えが報告されるか |
    |---|---|
    | ウィンドウを 1 枚も開いていない | **される**（25〜37 ms） |
    | ログウィンドウを開き、**WinRemap 自身のウィンドウ**へ切り替え | **される**（143〜168 ms） |
    | ログウィンドウを開き、**他のアプリ**へ切り替え | **されない**（10 秒待っても出ない） |

    UIA の読み取りと**スクリーンショットが一致する**ので、ウィンドウが見せていないのではなく報告が起きていない。さらに同じ状態で `C-h` を打つと `[decided] C-h (BS 0x08) → passed through` になり、**メモ帳に紐付いたルールが選ばれない**（メモ帳の文字は `abc` のまま。効いていれば `abcx`）。前面アプリ行を書く `refresh_foreground_cache` が**フックの読むキャッシュも更新している**ためで、行が出ないことと**アプリ別リマップが効かないことは同じ 1 つの事象**である
  - **v0.5 の「exe キャッシュ自体は追従している」という記述は取り消す。** `05-remap-notepad` が通るのは**その検査がウィンドウを 1 枚も開かないから**で、追従の証拠にはなっていなかった。**アプリ別キーマップが「特定の切り替えのときだけ効かない」と報告されたら、まずウィンドウが開いていたかを聞く**
