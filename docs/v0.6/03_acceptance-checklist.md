...
**H-1〜H-9 は [v0.5 §4](../v0.5/03_acceptance-checklist.md) と同一である。** 毎回そちらを開かずに済むよう再掲する。「v0.6 での見どころ」の列だけが本バージョン固有で、**その版で何が変わったから何を疑うか**を書いてある。**H-10 は本バージョンで追加した**（[ADR 0062](decisions/0062-detach-console-when-resident.md)）。
...
| **H-10** | **ターミナルから起動しても常駐が続くこと**（**v0.6 で追加**） | ターミナルで `winremap.exe` を実行 → メモ帳で `C-h` が効くことを確認 → **そのターミナルウィンドウを閉じる** → メモ帳で再び `C-h`。次に `--debug` 付きで同じことを行う | 無印は**閉じても効き続ける**、`--debug` は**閉じると終了する**（[ADR 0062](decisions/0062-detach-console-when-resident.md) の決定 3 のとおり）。ついでに `winremap.exe --config 存在しない.toml` のエラーが**ターミナルに出る**（ダイアログにならない）ことも見る — 決定 2 の確認 |
