...
| | §3 **P-8**（認定通過後） | **未実施。** 認定を通るまで測れない項目であり、これが済むまで v0.6 の受け入れは閉じない。**2026-07-30 に Partner Center へ提出済み・認定待ち**（[03_release-operations.md §4.2](../03_release-operations.md)） |
...
### この版で残った未確認（2026-07-29）

**P-8 だけが未実施である。** Store の認定を通るまで測れないため、**v0.6 の受け入れはまだ閉じていない**。認定通過後に実施し、この記録欄に追記する。それまでは「自動側全件緑・手動 H-1〜H-9 と P-1〜P-7 通過、ただし本バージョンの目的そのものは未検証」という状態である。
