...
| 2026-07-29 | `cargo fmt --check` / `cargo clippy --all-targets -- -D warnings` | **どちらも警告なし** |
| 2026-07-29 | `cargo test` | **114 件すべて通過**（89＋18＋7）。v0.5 の 108 件から 6 件増（[ADR 0061](decisions/0061-packaged-config-path.md) のパス写像 3 件と、[ADR 0059](decisions/0059-first-run-creates-the-default-config.md)・[ADR 0062](decisions/0062-detach-console-when-resident.md) 由来） |
| 2026-07-29 | `.\run-vm-ui-test.ps1`（全 9 件） | **全件 PASS**。`00-uia-actuation` 5/5、`00-cli-smoke` 8/8、`00-regression` 14/14、`00-log-view` 26/26、シナリオ `01-settings-window`〜`05-remap-notepad` も PASS。**v0.6.0 の受け入れはこの実行を正とする** |
| 2026-07-29 | §2 手動最小集合 **H-1〜H-9** | **オーナー確認済み・OK**（オーナー申告 2026-07-29）。実施者はオーナーで、本記録は申告に基づく |
| 2026-07-29 | §3 MSIX 固有 **P-1〜P-7** | **オーナー確認済み・OK**（オーナー申告 2026-07-29）。リリースビルドを `build.ps1 -Register` で登録して実施。**P-2 に注記あり**（下記） |
| 2026-07-31 | §3 **P-8**（Store 経由インストール） | **オーナー確認済み・OK**（オーナー申告 2026-07-31）。認定を通過してストアページが公開され、そこから実際にインストールして**警告が出ないことを確認した**。これで **v0.6 の受け入れは閉じた**（下記） |

### P-2 の注記（2026-07-29）

「テキストエディタで開く」を押しても**エディターが開かないように見えた**が、実際には **Zed が開いていた**（オーナー確認）。「フォルダーを開く」は Explorer が正常に開いた。したがって **P-2 は通過**である。

記録に残すのは、**押した直後に何も起きていないように見える**という観察のほうである。既定のハンドラーが起動に時間のかかるエディターだと、利用者は失敗と区別がつかない。今回はパッケージ版で試したために「リダイレクトのせいでパスが通らなかったのでは」と疑う筋道が最初に立ったが、**原因は WinRemap の外**だった。同種の報告を受けたときは、まずエディター側を確認する。

不具合ではないため修正はしない。v0.7 でこの周辺に触れるときに、押下のフィードバックを出すかどうかを検討する材料として残す。

### 受け入れの完了（2026-07-31）

**v0.6 の受け入れは閉じた。** 自動側は全件緑、手動は H-1〜H-9 と P-1〜P-8 がすべて通過している。

P-8 は 2 日遅れて実施した。Store の認定を通るまで測れない項目だったためである（§1「本当の Store 経由インストールは認定通過まで試せない」）。認定通過後、オーナーがストアページから実際にインストールし、**SmartScreen の警告が出ないことを確認した**。

これは**このバージョンが存在する理由そのもの**である（[開発計画 §0](01_development-plan.md)）。v0.6 は機能追加ではなく配布経路の版であり、「警告が出ない」が確認できなければ他が全部通っていても目的を達していない。逆に言えば、この 1 項目だけは受け入れの他のどの項目とも交換できない。

#### この段取りから残す教訓

**受け入れの一部がリリース後にしか測れないことがある。** v0.6 では P-8 がそれで、リリースを止めずに受け入れも閉じないという状態を 2 日間持った。同じ形はチャネルを増やすたびに起きる（winget のマージ待ちも同種である）。**測れない項目を「未実施」と明示して残し、リリースの完了と受け入れの完了を別々に扱う**のが、このプロジェクトでの扱い方になる。
