...
**撮影済み（2026-07-29）**: 日英各 4 枚を `packaging/msix/capture-screenshots.ps1` で取得した。ウィンドウの矩形だけを切り出して 1920×1080 の無地キャンバスに合成しているため、デスクトップの他の要素は含まれない。撮影時は対象ウィンドウを Z オーダーで最前面に固定する — 固定しないと手前のウィンドウが写り込む（実際に 1 度混入した）。

実際に採用したのは上表の 6 案のうち **1・2・3・4 の 4 枚**である。5（IME インジケーター）と 6（TOML の実ファイル）は、前者が任意機能・後者が設定ウィンドウ 01 と内容が重複するため見送った。掲載は最大 10 枚まで可能なので、必要なら後から足せる。
