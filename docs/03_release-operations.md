...
> **2026-07-30 実施（v0.6.0）**: オーナーが `packaging\msix\out\winremap-0.6.0.msix`（4.3 MB・未署名・バージョン `0.6.0.0`）を Partner Center へ提出した。**認定待ち**。提出前に 4.1 の順序制約は満たしている（`privacy.html` が日英とも 200）。手順 5 の確認と但し書きの削除、および[受け入れ](./v0.6/03_acceptance-checklist.md) **P-8 は認定通過後**であり、**それが済むまで v0.6 の受け入れは閉じない**。
