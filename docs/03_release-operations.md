...
5. **公開後に確認する**（本バージョンの目的そのもの）:
   ...
   >
   > **P-9 に手を付ける前に、次の 2 つを先にやること**（2026-08-10 に確立。この手当てで 0.8.0 の P-9 は守れた）。
   >
   > 1. **Microsoft Store の「アプリの更新」をオフにする**（Store → プロフィール → 設定）。**通過の当日でも、受け入れの手順を踏んでいる最中に自動更新へ先を越されうる。** 切っておけば、配信が行き渡るのを待っても P-9 は失われない。**P-8 まで終わったら戻すこと** — この設定は Store のすべてのアプリに効く
