...
- 色、余白、文字サイズ、ウィンドウの初期サイズは **`src/theme.rs` に集約**し、ウィジェットのコードに直接書かない（オーナー決定 2026-07-21）。「設定ウィンドウは何ピクセルで開くのか」「テーブルヘッダーの反転色は何か」を 1 ファイルで確認・変更できるようにするため
- 利用者が設定する項目ではない。**開発者向けのファイル**であり、TOML には出さない
- **egui 側の色は定数ではなく関数**にする。設定ウィンドウとログウィンドウはシステムのライト / ダークテーマに追従するため、RGB を固定すると片方のテーマで破綻する。`theme.rs` が固定するのは色そのものではなく**関係**（ヘッダー帯は文字色、ヘッダー文字は背景色 ＝「反転」）である
- **GDI オーバーレイ（マクロ記憶バナー、IME インジケーター）の色は固定**でよい。他アプリのウィンドウに重なる半透明パネルには、追従すべき背景が無いため
- アイコンは寸法を固定せず、**隣に置く文字の高さに合わせる**（`theme::button_icon_size` 等）。フォントスケールが変わっても釣り合いが崩れないようにするため
