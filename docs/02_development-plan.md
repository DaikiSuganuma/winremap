...
1. `docs/config-spec.md` 確定 — ブリーフ §6 の TOML 案を仕様化。キー記法（`C-` / `A-` / `S-` / `W-`、`Back` / `Enter` / `Esc` / `a`-`z` / `F1`-`F24` 等）と VK コード対応表を定義
2. `keymap.rs`
...
- 手動テスト結果を `docs/acceptance-checklist.md` に記録（Phase 4 でリリース用チェックリストに発展させる）
...
5. 設定エラー時の挙動確定: 起動時エラーは通知して終了 or 空マップで常駐、リロード時エラーは旧設定を維持して通知（仕様として config-spec.md に明記）
...
7. `docs/acceptance-checklist.md` 完成版で手動受け入れテスト全 5 項目（ブリーフ §9）を実施・記録
