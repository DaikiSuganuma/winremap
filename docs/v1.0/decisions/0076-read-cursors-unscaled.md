...
- **プローブの復元側の検査は、いまも矢印しか測っていない**（`startup-clears-any-leftover`・`restores-when-the-ime-goes-off`・`the-next-start-restores-it`、および繰り返し検査のオフ側）。I ビームを測るのは IME がオンの間だけである。本 ADR の範囲外だが、**「復元したあと I ビームが空で残る」は現在の 10 項目を全部通過する**
  - **追記 2（2026-08-16 午後）— プローブが測っていたのは、実はホストだった。** 同じ 10 項目が同じバイナリのまま、朝は全部通り、午後は 6 件落ちた。**プローブを走らせる PowerShell が DPI 非対応から対応へ変わっていた**（`GetAwarenessFromDpiAwarenessContext` が 0 → 1、`SM_CXCURSOR` が 32 → 48）。48 で読めば素の I ビームは存在しないので、**健全な機械について「I ビームが空だ」と報告する**。実測（WinRemap を 1 つも動かさない状態）: 48 で読むと描画 0 画素、32 で読むとマスク専用の 26 画素。**読み取りを `SetThreadDpiAwarenessContext` で DPI 非対応に固定した**（プロセス単位は既に対応済みのホストでは拒否される）。**この ADR が言っていることが、この ADR を検証する道具自身にも起きていた。**
  - **追記（2026-08-16、オーナー指示により対応済み）**: 各カーソルの「描かれている画素数」を数えるようにし、上記 4 か所すべてで復元後の I ビームに 2 つの問い（着色が落ちている・カーソルがそこに在る）を立てるようにした。同日の実測: 0.9.0 が稼働している状態で、セッションに登録されている I ビームは 32×32 のカラーカーソルで**描画 0 画素** — 新しい検査はこれを落とす。この空は DPI 非対応の PowerShell から読んでも空であり、**セッションのカーソル表そのものが空になっている**ことを示す（同じ理由で、単体テストの DPI 2 件はこの状態では測るものが無い。飛ばして、そう言うようにした）
