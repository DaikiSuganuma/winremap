...
#### 2026-08-04 の訂正: 決定 3 の表が誤っていた

**上表の `cmd /c` の行は誤りである。** 翌日 2026-08-04 の受け入れで C-4 を書き換えた手順どおりに実施したところ、**`--debug` の専用ウィンドウが開き、ファイルは 0 バイトのままだった**。決定 3 の結論（「PowerShell の `>` は直せないので手順のほうを直す」）は変わらないが、**差し替え先の手順が誤っていた**。

理由は PowerShell とは別である。cmd は `>` を**自分の標準ハンドルを一時的に差し替える**やり方で実装しており、`CreateProcess` に `STARTF_USESTDHANDLES` を渡さない。**Windows は GUI サブシステムの子プロセスには、その経路で標準ハンドルを渡さない** — 子から見ると標準出力は未設定であり、`stdout_is_captured()` は正しく「リダイレクトされていない」と答え、設計どおり自前のコンソールを開く。**WinRemap 側に不具合は無い。**

| 経路 | 標準出力が届くか | 結果 |
|---|---|---|
| PowerShell の `>` | 届く（パイプ） | ファイルは空。窓は開かない。常駐は続く（この ADR の決定 2 の効き目） |
| `cmd /c "… > log.txt"` | **届かない** | **窓が開き**、ファイルは空 |
| `Start-Process -RedirectStandardOutput` | 届く | **通る**（442 バイト・5 行を確認） |
| Git Bash の `>` | 届く | 通る（MSYS は `STARTF_USESTDHANDLES` で渡す） |

**この誤りを生んだ手続きのほうも記録しておく。** 08-03 の調査でこの行を書いたとき、`cmd /c` を実際に通した記録が無い（届く経路を Git Bash で確かめ、`cmd /c` と書いてしまった可能性が高い）。**確かめずに書いた行が、翌日の受け入れで人の時間を使わせた。** 判定は `--help` を流すだけで足りる — ヘルプは必ず何か出すので、**0 バイトなら届いていない**。

C-4 の手順は `Start-Process -RedirectStandardOutput` に直した（[v0.8 §2.2](../03_acceptance-checklist.md#22---debug-の専用コンソールadr-0068)）。
