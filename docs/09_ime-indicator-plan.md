...
## 検証記録（Phase I3・2 回目: 2026-07-19）

- 実施: オーナー（ADR 0021/0022 の修正適用後）
- 結果: **OK — パネルが正常に表示されることを確認**。Phase I3 完了
- 設計書 §7 の残りの手動受け入れ項目（クリック透過・フォーカス非奪取・ウィンドウ切替追随・無効化リロード・マルチモニター等）は Phase I4 の受け入れテストで実施する

## 実使用フィードバック（Phase I4: 2026-07-19）

オーナーの実使用で 2 件の改善要望。[ADR 0023](decisions/0023-ime-indicator-query-target.md) で対処。

1. **タスクバークリックで表示される** → シェル面（`Shell_TrayWnd` / `Shell_SecondaryTrayWnd` / `Progman` / `WorkerW`）は照会・表示・状態更新のすべてを無視するようにした
2. **設定アプリの「設定の検索」で表示されない** → UWP ホストの子 `Windows.UI.Core.CoreWindow` を照会対象にするフォールバックを追加（`--debug` に「CoreWindow 経由」と表示）。ベストエフォートであり、受け入れチェックリスト項目 9 でオーナーが確認する
