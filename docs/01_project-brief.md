...
*最終更新: 2026-07-21（§3.3 のキーボードマクロを「設定内スクリプティング」に限定し実行時のマクロ記録・再生を例外採用、§5-2 に例外 4・§5-3 に `macro_record/banner.rs` を追加）*
