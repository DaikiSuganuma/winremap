...
- **設定ファイル内のスクリプティング**（設定はあくまで宣言的な TOML。条件分岐・変数・任意コードの実行を設定に持ち込まない）
  - 2026-07-21 改訂（オーナー指示）: **実行時のキーボードマクロ記録・再生**（記憶キーで押した操作を覚え、再生キーで送出する）は例外として採用する（[ADR 0043](v0.3/decisions/0043-runtime-macro-recording.md)）。記録内容はメモリ上のみでプロセス終了時に破棄し、ディスクへは書かない（§5-6 のキーロガー化禁止との整合）。設定ファイルは宣言的なまま（記憶するのはキーであってスクリプトではない）で、**設定内スクリプティングは引き続き Non-goal**
...
| `hook.rs` | フック設置/解除、コールバック。**unsafe は §5-3 の許可ファイルのみに閉じ込める** |
| `notify.rs` | ターミナル起動時の親コンソール接続と、コンソールが無い場合の通知ダイアログ（ADR 0029・0031） |
| `gui/` | 設定ウィンドウとログウィンドウ。1 つのイベントループを不可視ホストが持ち、実ウィンドウは子ビューポート（ADR 0035・0037）。`gui/win32.rs` のみ unsafe（ADR 0038） |
| `clock.rs` | ログのセッション境界に出すローカル時刻（`GetLocalTime`。std にローカル時刻が無いため。ADR 0041） |
...
3. **unsafe の隔離**: `unsafe` ブロックは `hook.rs` / `sender.rs` / `window.rs`、IME インジケーター関連の `src/ime_indicator/`（detect.rs / overlay.rs）と検証用 `examples/ime_probe.rs`（ADR 0020）、コンソール接続・通知ダイアログの `notify.rs`（ADR 0031）、GUI のウィンドウアイコン・関連付け起動の `src/gui/win32.rs`（ADR 0038）、およびログのセッション境界用ローカル時刻の `src/clock.rs`（ADR 0041）のみに置く。各 unsafe には安全性の根拠コメント（`// SAFETY:`）を必須とする
...
│   ├── 02_rust-guidelines.md   # Rust 開発の作法
│   ├── 04_config-spec.md       # §6 の確定仕様
│   ├── 05_acceptance-checklist.md  # §9 の手動テスト記録
│   ├── 03_release-operations.md    # §10 の運用手順（オーナー向け）
...
*最終更新: 2026-07-21（§3.3 のキーボードマクロを「設定内スクリプティング」に限定し、実行時のマクロ記録・再生を例外採用）*
