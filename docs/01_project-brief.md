...
3. **unsafe の隔離**: `unsafe` ブロックは `hook.rs` / `sender.rs` / `window.rs`、IME インジケーター関連の `src/ime_indicator/`（detect.rs / overlay.rs）と検証用 `examples/ime_probe.rs`（ADR 0020）、およびコンソール接続・通知ダイアログの `notify.rs`（ADR 0031）のみに置く。各 unsafe には安全性の根拠コメント（`// SAFETY:`）を必須とする
