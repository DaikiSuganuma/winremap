...
3. **unsafe の隔離**: `unsafe` ブロックは `hook.rs` / `sender.rs` / `window.rs`、IME インジケーター関連の `src/ime_indicator/`（detect.rs / overlay.rs）と検証用 `examples/ime_probe.rs`（ADR 0020）、コンソール接続・通知ダイアログの `notify.rs`（ADR 0031）、および GUI のウィンドウアイコン・関連付け起動の `src/gui/win32.rs`（ADR 0038）のみに置く。各 unsafe には安全性の根拠コメント（`// SAFETY:`）を必須とする
