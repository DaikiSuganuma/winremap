...
## 修正（2026-08-01、オーナー決定「v0.7 で直す」）

[ADR 0065](../decisions/0065-foreground-window-from-the-event.md) のとおり `hwnd` を使う形に直した。**同じスクリプト・同じ手順で測り直した結果**（変えたのはバイナリだけ）:

| 条件 | 修正前 | 修正後 |
|---|---|---|
| ウィンドウ無し（12 回） | 1 件 | **0 件** |
| ログウィンドウ有り（12 回） | 1 件 | **0 件** |

修正前の測定はこの前の節（18 件中 4 件）とは別の実行で、同じ道具・同じ回数の条件に揃えたものである。**発生率は 22% と 8% で揺れる** — 競合状態なので当然で、だからこそ「1 回試して緑」を根拠にしてはいけない。
