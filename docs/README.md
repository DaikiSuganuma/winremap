...
| [v1.1/](v1.1/) | 受け入れチェックリスト（[03_acceptance-checklist.md](v1.1/03_acceptance-checklist.md)）、decisions/（ADR 0080-0081）。**開発中。スコープ未定** — 現時点で入っているのは v1.0.0 のリリース後に気づいたトレイまわりの 2 件（アイコンが青い塗りつぶしに見える／表示名が小文字の `winremap`）だけで、開発計画（`01_development-plan.md`）はまだ無い |
