...
| [v1.0.1/](v1.0.1/) | 受け入れチェックリスト（[03_acceptance-checklist.md](v1.0.1/03_acceptance-checklist.md)）、decisions/（ADR 0080-0081）。**開発中。修正だけの版** — v1.0.0 のリリース後に気づいたトレイまわりの 2 件（アイコンが青い塗りつぶしに見える／表示名が小文字の `winremap`）。利用者から見える新機能が無いので SemVer どおりパッチとする（オーナー決定 2026-08-17）。**この版に開発計画（01）は無い** — 計画を書くほどのスコープが無いことがパッチである理由なので、**フォルダーが 3 桁なのは初めてだが正しい** |
