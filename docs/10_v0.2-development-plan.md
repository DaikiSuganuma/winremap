...
> 元資料: [01_project-brief.md](./01_project-brief.md) §3.2（v0.2 候補）、[02_development-plan.md](./02_development-plan.md) Phase 5。
...
| expand-region（選択範囲の段階的拡大、[検討ノート](notes/20260719_expand-region-study.md)） | 不変条件の例外リスト改訂が必要になる規模に対し需要が小さい |
...
推奨は **A + B の併用**: 通常起動は無音（エラーはトレイ通知またはメッセージボックス）、ターミナル起動時は親コンソールへ出力、`--debug` をエクスプローラーから起動された場合のみ `AllocConsole`。ファイル出力（C）は不変条件 6 との整合を考え、v0.2 では採用しない。
...
1. ADR 0029 — ログ出力方式の選定（上表の比較と採用理由・却下理由）
2. `#![windows_subsystem = "windows"]` 化と console attach/alloc の実装
   - 注意: `AttachConsole` 等の unsafe 呼び出しの配置は不変条件 3 の unsafe 隔離リスト（hook.rs / sender.rs / window.rs / ime_indicator）に含まれない。ADR 0009（bootstrap unsafe の配置）の改訂または ADR 0029 での明示的な例外追加が必要（**オーナー承認事項**）
3. `--help` / `--version` / 設定エラー時の動作確認と通知先の再設計（無音起動時のエラーはメッセージボックスまたはトレイバルーンで通知）
4. インストーラー（Inno Setup）・ショートカット経由の起動確認
5. `docs/05_acceptance-checklist.md` に確認項目を追加
...
2. `docs/11_config-gui-design.md` — 画面設計・編集モデルの設計書
...
2. `docs/05_acceptance-checklist.md` v0.2 項目で手動受け入れテスト実施・記録
...
| フェーズ | 判断事項 |
|---|---|
| A | ログ出力方式の採用案（推奨: AttachConsole + `--debug` 時 AllocConsole）、unsafe 隔離リストの例外追加 |
