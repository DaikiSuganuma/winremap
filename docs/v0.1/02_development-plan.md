...
3. ホットリロード — v0.1 はトレイメニューからのリロードのみ（ファイル監視は v0.2 へ延期、ADR 0008）。リロードは arc-swap でキー取りこぼしなし（§9-4）
...
> 2026-07-18: オーナー指示により、除外リスト（ADR 0011）・マクロ出力（ADR 0012）・2 ストロークシーケンス（ADR 0013）の 3 機能を v0.1.0 公開前に前倒しで導入した。
>
> 2026-07-19: オーナー指示により、IME 状態インジケーターの統合を決定（ADR 0020）。設計・計画は [08_ime-indicator-design.md](./08_ime-indicator-design.md) / [09_ime-indicator-plan.md](./09_ime-indicator-plan.md) に分離して管理する。
