...
| `cursor` | 真偽値 | — | `false` | **IME がオンの間、マウスカーソルに色を付ける**（[ADR 0067](../v0.8/decisions/0067-ime-cursor-color.md)）。矢印とテキストの I ビームの 2 つが対象。`enabled` とは独立で、パネルを出さずにカーソルだけ使える |
| `cursor_color` | 文字列 | `#rrggbb` | `#e05a2b` | 上の色。カーソルの形はそのままに**明るさに応じて着色**するので、黒い輪郭は黒のまま、白い部分がこの色になる |

- `cursor` は**セッション全体のカーソルを置き換える**（`SetSystemCursor`）。WinRemap の中だけの変更ではない
