...
- 2026-09-15、オーナーから Store の認定を通ったと連絡があった
- 開発者登録した 1.0.1 のパッケージ専用フォルダーの `personal-ja.toml`（6320 バイト）と `last-config.txt`（60 バイト）を `C:\Users\suganuma\winremap-dev-1.0.1-backup-20260915` へ複製し（ハッシュ一致）、`Remove-AppxPackage` で削除した。パッケージ・パッケージ専用フォルダー・プロセスがどれも無くなったことを確かめた
- オーナーが Microsoft Store からインストールした。`Get-AppxPackage` は `Version 1.0.1.0`・`SignatureKind Store`・インストール先 `C:\Program Files\WindowsApps\SUGANUMADaiki.WinRemap_1.0.1.0_x64__pktmgf1zdhxe0`。起動の前に、退避した 2 ファイルを新しいパッケージ専用フォルダーへ戻した（ハッシュ一致）
- オーナーがスタート メニューから起動した。プロセスは上記インストール先の `winremap.exe`（7:41:01 起動）。トレイの行が新しく作られ、ツールチップは「WinRemap — キーマップ 5 件」
- オーナーの報告: ①インストールから起動まで SmartScreen が出ない ②Store のページの発行元が SUGANUMA Daiki ③トレイに常駐し、メモ帳で `C-h` が 1 文字削除になる
- 2026-09-14 に「測れない」とした P-8 を、この日に測った。§6 の表に行を追記する
...
| P-8 | 通過 | 2026-09-15、認定通過後に Store からインストールした 1.0.1（`SignatureKind Store`）で、①インストールから起動まで SmartScreen が出ない ②発行元が SUGANUMA Daiki ③トレイに常駐し、メモ帳で `C-h` が 1 文字削除になる、との報告。上の「測れない」の行（2026-09-14）を置き換える |
