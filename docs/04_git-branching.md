...
| release | **`release/*`** | リリース準備。CHANGELOG の切り出し、受け入れテストの記録（バージョン更新は §2.6 のとおり開発開始時に済ませる） |
...
リリースの合間に、ドキュメント・サイトだけを `develop` から `main` へ入れる経路がもう 1 本
ある（§2.5.1）。タグは打たない。
...
- `main` へのマージは `release/*`・`hotfix/*`・`develop` の 3 つ。**トピックブランチを
  直接入れない**（`develop` 経由の条件は §2.5.1）
...
### 2.5.1 リリースの合間のドキュメント・サイト修正は `develop` 経由で `main` へ入れてよい

**オーナー決定（2026-07-31）。** 条件は 1 つで、**マージコミットを読めば何をしたのかが
分かること**である。

対象は**リリース成果物に含まれないファイル**に限る。

| 入れてよい | 入れない |
|---|---|
| `README.md` / `README.ja.md`、`SECURITY.md` 等のリポジトリ直下の文書 | `src/`、`Cargo.toml` / `Cargo.lock`、`build.rs` |
| `site/`（GitHub Pages） | `installer/`、`packaging/`、`.github/workflows/` のうちリリースの挙動を変えるもの |
| `docs/` | |

分ける理由は §2.3 そのものである。`develop` を置いたのは **`main` を clone した人が
公開したことのないコードを手にするのを防ぐ**ためであり、その目的はコードを送らない限り
損なわれない。exe が 1 バイトも変わらない修正のために `hotfix/*` を切って版番号を上げると、
**中身の同じ 0.6.1 を全利用者に配ること**になる（Store 版は自動更新される）。

なぜ待てないかというと、`README.md` は GitHub のトップページに出るものであり、`site/` は
`main` からのみ Pages に発行されるためである。次のリリースまで数か月あることは普通にあり、
その間ずっと**トップページと配布ページが間違ったことを言い続ける**。

手順は通常のマージと同じ。

```powershell
git checkout main
git merge --no-ff develop -m "Merge develop into main: ストアの掲載を反映し、認定待ちの但し書きを消す"
git checkout develop
git merge --no-ff main     # main 側のマージコミットを develop に戻す
```

> **前例**: この運用はルール制定より前から行われていた。`71c9b20`（winget/scoop の
> パッケージングとインストール手順）と `3ae537f`（scoop 保留の反映）はどちらも
> リリース間のドキュメント・サイト変更で、git-flow を採用した `ce6b778` より**後**である。
> v0.6 でも同じことをした（[v0.6 開発計画 §7.5](v0.6/01_development-plan.md)）。
> 条文のほうが運用に追いついていなかったので、条文を直した。

### 2.6 バージョン番号は開発開始時に上げる（原典との相違）

**`Cargo.toml` の `version` は、そのバージョンの開発を始める時点で上げる。**
原典（および一般的な git-flow の運用）はリリースブランチ上で上げるが、本プロジェクトは
そうしない。

理由は、**手元のビルドが「テスト中のもの」か「公開済みのもの」か分からなくなる**ためである。
リリース時まで番号を据え置くと、v0.3 の開発中ビルドも公開済みの v0.2.0 も等しく
`WinRemap v0.2.0` と名乗る。この表示はトレイメニューの先頭行・`--version`・ログの
起動バナーに出るため、手動受け入れテストの最中に「今どちらを触っているのか」が
分からなくなる。オーナー 1 名が同じマシンで開発と検証を行う本プロジェクトでは、
これは実際に起きる混乱である（オーナー決定 2026-07-21）。

- 開発開始時（新バージョンの最初の作業ブランチ）で `Cargo.toml` を次の番号に上げる
- `release/*` 上ではリリースする番号になっていることを**確認するだけ**にする
  （[03_release-operations.md](03_release-operations.md) §2-3）
- 代償は、タグを打つまでの間「まだ存在しないバージョン」を名乗るビルドが手元にあること。
  ただし配布物は Releases のタグ付きビルドのみであり（ブリーフ §10-3）、
  外に出る成果物がこの状態になることはない
